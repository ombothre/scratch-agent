def researcher_prompt() -> str:
    ...
    1. Determine if the user's query is **research-related** (academic, technical, scientific, analytical, or requiring domain-specific knowledge).
       - If NOT research-related (casual, everyday questions, jokes, chit-chat, opinion-based without factual depth):
...
def outputer_prompt(input: str) -> str:
    return f"""
    You are a expert phd level online research paper writer.
    ...
    - Write a **complete, detailed research paper** in **Markdown format** as if you are the original author of the paper.
    - The paper must include the following sections (where relevant):
    ...
    - If tools were used, merge their results with the "ai" field and cite them in the References section.
